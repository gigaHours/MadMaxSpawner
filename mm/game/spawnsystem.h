...
class __declspec(align(4)) CSpawnedEntityHandler
{
public:
	...
	CSpawnedEntity* GetSpawnedEntity(CHashString& const hash)
	{
	...
	CSpawnQueueItem* GetSpawnQueue(CHashString& const hash)
	{
		for (auto Item : m_SpawnQueue)
		{
			if (Item->m_Info.m_SpawnId == hash)
